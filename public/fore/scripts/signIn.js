        ...
        $('.text>ul>li').click(function () {
          ...
          if (options.CanClick == 1) {
            ...
            for (var i = 0; i < options.res.length; i++) {
              ...
              Numr = Numl + parseInt(options.res[j].pro)
            ...
            // 将剩余选项随机分配
            for (var s = 0; s < $('.text>ul>li').length; s++) {
              if ($('.text>ul>li').eq(s).attr('class') !== 'active') {
                $('.text>ul>li').eq(s).find('span').text(OtherList[RandomNum])
                RandomNum++
              }
            }
